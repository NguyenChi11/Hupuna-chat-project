...
import { ChatFlashItem } from './types';
...
type Props = {
  ...
  removeItemFromTopic: (topicId: string, messageId: string) => void;
  onRenameItem?: (topicId: string, itemId: string, newName: string) => void;
};
...
export default function TopicList({
  ...
  const renderContentItem = (it: ChatFlashItem, idx: number, topicId: string): React.ReactNode => {
        ...
        return (
          ...
          >
            {kind === 'video' ? (
              ...
              <Image width={200} height={200} src={String(url)} alt="" className="w-36 h-36 object-cover" />
            ) : (
              <Image width={200} height={200} src={getProxyUrl(url)} alt="" className="w-36 h-36 object-cover" />
            )}
            ...
            <ItemDropdownMenu
              ...
              onRemoveFromFolder={(mid) => removeItemFromTopic(topicId, mid)}
            />
          </div>
        );
      }
    ...
    if (isImg || isVid) {
      return (
        ...
        >
          {isVid ? (
            <video src={getProxyUrl(fileUrl)} className="w-36 h-36 object-cover pointer-events-none" preload="metadata" />
          ) : String(fileUrl).startsWith('blob:') ? (
            <Image width={200} height={200} src={String(fileUrl)} alt="" className="w-36 h-36 object-cover" />
          ) : (
            <Image width={200} height={200} src={getProxyUrl(fileUrl)} alt="" className="w-36 h-36 object-cover" />
          )}
          <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
  ...
  const currentItems = selectedTopicId ? items || [] : [];
  const classify = (it: ChatFlashItem) => {
    const msg = messages.find((m) => String(m._id) === String(it.id));
    if (msg) {
      const fileUrl = String(msg.fileUrl || msg.previewUrl || '');
      const isVid = msg.type === 'video' || isVideoFile(fileUrl) || isVideoFile(msg.fileName);
  ...
  if (!selectedTopicId) return <div className="text-sm text-gray-500">Chọn một chủ đề ở sidebar</div>;
  if (currentItems.length === 0) return <div className="text-sm text-gray-500">Chủ đề chưa có nội dung</div>;
  return (
        ...
        <div>
          ...
          {mediaItems.length > 0 && (
            <div className="grid grid-cols-3 md:grid-cols-4 gap-2 mb-2">
              {mediaItems.map((it, idx) => renderContentItem(it, idx, selectedTopicId))}
            </div>
          )}
          {fileItems.length > 0 && (
            <div className="space-y-2">{fileItems.map((it, idx) => renderContentItem(it, idx, selectedTopicId))}</div>
          )}
        </div>
      )}
      {textItems.length > 0 && (
        <div>
          ...
          <div className="space-y-2">{textItems.map((it, idx) => renderContentItem(it, idx, selectedTopicId))}</div>
        </div>
      )}
    </div>
  );
